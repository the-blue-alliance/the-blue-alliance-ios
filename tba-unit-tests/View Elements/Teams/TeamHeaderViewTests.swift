...
class TeamHeaderViewModelTests: TBATestCase {
    ...
    override func setUp() {
        ...
        team.teamNumber = NSNumber(value: 7332)
    }

    func test_no_nickname_no_year_no_avatar() {
        let vm = TeamHeaderViewModel(team: team, year: nil)

        XCTAssertEqual(vm.teamNumber, "Team 7332")
        XCTAssertNil(vm.nickname)
        XCTAssertFalse(vm.hasNickname)
        XCTAssertNil(vm.avatar)
        XCTAssertFalse(vm.hasAvatar)
        XCTAssertEqual(vm.year, "----")
    }
    ...
    func test_no_nickname_no_year_avatar() {
        ...
        avatar.year = 2018
        avatar.type = MediaType.avatar.rawValue
        team.addToMedia(avatar)
