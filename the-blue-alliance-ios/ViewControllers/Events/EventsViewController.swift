...
@MainActor protocol EventsViewControllerDelegate: AnyObject {
    func eventSelected(_ event: Event)
}

class EventsViewController: TBACollectionViewListController<EventCollectionViewListCell, Event> {

    // MARK: - Events View Controller Configuration

    class var firstEventSortKeyPathComparators: [KeyPathComparator<Event>] {
    ...
    var events: [Event]? = nil {
        didSet {
            events?.sort(using: Self.firstEventSortKeyPathComparators + [
    ...
    init(dependencyProvider: DependencyProvider) {
        super.init(headerMode: .supplementary, dependencyProvider: dependencyProvider)
    }
