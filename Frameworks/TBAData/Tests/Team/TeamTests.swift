...
class TeamTestCase: TBADataTestCase {
    ...
    func test_insert_page() {
        ...
        Team.insert([modelOne, modelTwo], page: 0, in: viewContext)
        XCTAssertNoThrow(try viewContext.save())

        let teamsFirst = Team.fetch(in: viewContext)
        ...
        Team.insert([modelThree], page: 1, in: viewContext)
        XCTAssertNoThrow(try viewContext.save())

        let teamsSecond = Team.fetch(in: viewContext)

        XCTAssertEqual(teamsSecond.count, 3)

        Team.insert([], page: 1, in: viewContext)
        XCTAssertNoThrow(try viewContext.save())

        let teamsThird = Team.fetch(in: viewContext)

        XCTAssertEqual(teamsThird.count, 2)

        Team.insert([modelTwo], page: 0, in: viewContext)
        XCTAssertNoThrow(try viewContext.save())

        let teamsFour = Team.fetch(in: viewContext)
    ...
    func test_insert() {
        ...
        let team = Team.insert(model, in: viewContext)
    ...
    func test_insert_events() {
        ...
        let team = Team.insert(teamModel, in: viewContext)
    ...
    func test_update() {
        ...
        let teamOne = Team.insert(modelOne, in: viewContext)

        let modelTwo = TBATeam(key: "frc7332", teamNumber: 7332, name: "New Name", rookieYear: 2011)
        let teamTwo = Team.insert(modelTwo, in: viewContext)
    ...
    func test_delete() {
        ...
        let media = TeamMedia.insert(mediaModel, year: 2010, in: viewContext)
