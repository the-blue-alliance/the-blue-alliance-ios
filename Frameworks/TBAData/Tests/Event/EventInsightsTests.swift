...
class EventInsightsTestCase: TBADataTestCase {
    ...
    func test_insert() {
        ...
        let insights = EventInsights.insert(model, eventKey: event.key!, in: viewContext)
    ...
    func test_update() {
        ...
        let insightsOne = EventInsights.insert(modelOne, eventKey: event.key!, in: viewContext)
        insightsOne.event = event

        let modelTwo = TBAEventInsights(qual: nil, playoff: nil)
        let insightsTwo = EventInsights.insert(modelTwo, eventKey: event.key!, in: viewContext)
    ...
    func test_isOrphaned() {
        let insights = EventInsights.init(entity: EventInsights.entity(), insertInto: viewContext)
        XCTAssert(insights.isOrphaned)

        insights.event = coreDataTestFixture.insertDistrictEvent()
