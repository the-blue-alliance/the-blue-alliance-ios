...
class EventTestCase: TBADataTestCase {
    ...
    func test_insert_year() {
        ...
        Event.insert([modelEventOne, modelEventTwo], year: 2018, in: viewContext)
        let eventsFirst = Event.fetch(in: viewContext) {
        ...
        Event.insert([modelEventTwo], year: 2018, in: viewContext)
        let eventsSecond = Event.fetch(in: viewContext) {
            $0.predicate = NSPredicate(format: "%K == %ld",
                                       #keyPath(Event.year), 2018)
        }
    ...
    func test_insert() {
        ...
        let event = Event.insert(eventModel, in: viewContext)
    ...
    func test_insert_awards_teamKey() {
        ...
        let frc1TeamKey = TeamKey.findOrFetch(in: viewContext, matching: NSPredicate(format: "%K == %@", #keyPath(TeamKey.key), frc1Model.teamKey!))!

        XCTAssertNoThrow(try viewContext.save())

        let awardsFirst = Award.fetch(in: viewContext) {
        ...
        let awardsSecond = Award.fetch(in: viewContext) {
            $0.predicate = NSPredicate(format: "SUBQUERY(%K, $r, $r.teamKey.key == %@).@count == 1",
                                       #keyPath(Award.recipients), frc1TeamKey.key!)
        }
    ...
    func test_hybridType_district() {
        ...
        let district = District(entity: District.entity(), insertInto: viewContext)
    ...
    func test_dateString_noDates() {
        let noDates = Event.init(entity: Event.entity(), insertInto: viewContext)
    ...
    func test_dateString() {
        let sameDay = Event.init(entity: Event.entity(), insertInto: viewContext)
        ...
        let sameYear = Event.init(entity: Event.entity(), insertInto: viewContext)
        ...
        let differentYear = Event.init(entity: Event.entity(), insertInto: viewContext)
