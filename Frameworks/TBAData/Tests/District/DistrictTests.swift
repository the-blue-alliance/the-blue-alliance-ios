...
class DistrictTestCase: TBADataTestCase {
    ...
    func test_insert_year() {
        ...
        District.insert([modelDistrictOne, modelDistrictTwo], year: 2018, in: viewContext)
        let districtsFirst = District.fetch(in: viewContext) {
        ...
        District.insert([modelDistrictTwo], year: 2018, in: viewContext)
        let districtsSecond = District.fetch(in: viewContext) {
            $0.predicate = NSPredicate(format: "%K == %ld",
                                       #keyPath(District.year), 2018)
        }
    ...
    func test_insert() {
        ...
        let district = District.insert(modelDistrict, in: viewContext)
    ...
    func test_update() {
        ...
        let duplicateDistrict = District.insert(duplicateModelDistrict, in: viewContext)
    ...
    func test_delete() {
        ...
        let ranking = DistrictRanking(entity: DistrictRanking.entity(), insertInto: viewContext)
        ranking.district = event.district

        viewContext.delete(district)
    ...
    func test_isHappeningNow_notCurrentYear() {
        ...
        let dcmp = Event.init(entity: Event.entity(), insertInto: viewContext)
