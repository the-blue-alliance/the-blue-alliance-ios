...
class DistrictRankingTestCase: TBADataTestCase {
    ...
    func test_insert() {
        ...
        let districtRanking = DistrictRanking.insert(modelDistrictRanking, districtKey: event.district!.key!, in: viewContext)
    ...
    func test_update() {
        ...
        let eventNew = coreDataTestFixture.insertDistrictEvent(eventKey: "2018mike2")
        ...
        let duplicateDistrictRanking = DistrictRanking.insert(duplicateModelDistrictRanking, districtKey: event.district!.key!, in: viewContext)
    ...
    func test_delete() {
        ...
        viewContext.delete(districtRanking)
        XCTAssertNoThrow(try viewContext.save())
    ...
    func test_sortedEventPoints() {
        ...
        let eventOne = Event.init(entity: Event.entity(), insertInto: viewContext)
        eventOne.startDate = Event.dateFormatter.date(from: "2018-03-01")!
        eventOne.key = "2018miket"

        let eventTwo = Event.init(entity: Event.entity(), insertInto: viewContext)
        eventTwo.startDate = Event.dateFormatter.date(from: "2018-03-02")!
        eventTwo.key = "2018mike2"

        let eventThree = Event.init(entity: Event.entity(), insertInto: viewContext)
        eventThree.startDate = Event.dateFormatter.date(from: "2018-03-03")!
        eventThree.key = "2018mike3"

        let ranking = DistrictRanking.init(entity: DistrictRanking.entity(), insertInto: viewContext)
        ...
        ranking.addToEventPoints(NSSet(array: [modelEventPointsTwo, modelEventPointsThree, modelEventPointsOne].map({
            return DistrictEventPoints.insert($0, in: viewContext)
        })))
    ...
    func test_isOrphaned() {
        ...
        let district = District.init(entity: District.entity(), insertInto: viewContext)
