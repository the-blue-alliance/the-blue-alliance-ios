...
class DistrictEventPointsTestCase: TBADataTestCase {
    ...
    func test_insert() {
        ...
        let eventPoints = DistrictEventPoints.insert(modelEventPoints, in: viewContext)
    ...
    func test_insert_event() {
        ...
        DistrictEventPoints.insert([modelPointsOne], eventKey: event.key!, in: viewContext)
        let pointsOne = DistrictEventPoints.fetch(in: viewContext) {
        ...
        DistrictEventPoints.insert([modelPointsTwo], eventKey: event.key!, in: viewContext)
        let pointsTwo = DistrictEventPoints.fetch(in: viewContext) {
            $0.predicate = NSPredicate(format: "%K == %@",
                                       #keyPath(DistrictEventPoints.eventKey.key), event.key!)
        }.first!
    ...
    func test_update() {
        ...
        let duplicateEventPoints = DistrictEventPoints.insert(duplicateModelEventPoints, in: viewContext)
    ...
    func test_isOrphaned() {
        let eventPoints = DistrictEventPoints.init(entity: DistrictEventPoints.entity(), insertInto: viewContext)
        ...
        event.key = key
        let eventKey = EventKey.insert(withKey: key, in: viewContext)
