...
class SearchableTestCase: XCTestCase {
    ...
    func test_relevantShortcut_event_none() {
        let event = Event(context: persistenceContainer.viewContext)
        event.keyRaw = "2022miket"
    ...
    func test_relevantShortcut_event_date() {
        let event = Event(context: persistenceContainer.viewContext)
        event.keyRaw = "2022miket"
        event.startDateRaw = Event.dateFormatter.date(from: "2022-03-18")!
        event.endDateRaw = Event.dateFormatter.date(from: "2022-03-21")!
